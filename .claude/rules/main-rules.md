...
Imperative guidance for working on `django-health-check-full-of-juice`. Follow these end-to-end on every change.
...
- When you see `getLogger(__name__)`, change it to `getLogger("django_health_check_full_of_juice")`.
...
- **Selective runs**: replace `<test_module>` with the dotted path of the test module you want to run (e.g. `tests.test_commands`):
    ```bash
    ...
    ```
- **Coverage for selective testing**: replace `<test_module>` with the dotted path of the test module and `<path/to/source_file>.py` with the source file you want a coverage report on (e.g. `health_check/management/commands/health_check.py`). Multiple files can be passed comma-separated:
    ```bash
...
- Update `CHANGELOG.md` only when `./health_check/` (the library source) changes, but not including the `tests` folder. Use the active `[X.Y.Z]` heading and `Added` / `Changed` / `Fixed` / `Removed` subsections. Skip it for repo-tooling or sample-project edits.
